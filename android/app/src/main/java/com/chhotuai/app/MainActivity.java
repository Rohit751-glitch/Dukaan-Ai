...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        if (saved != null && saved.startsWith("https://")) loadRemote(saved); else showConnector("");
    }

    private void loadRemote(String url) { loadingRemote = true; webView.loadUrl(url); }
        ...
        @JavascriptInterface public void connect(String rawUrl) {
            runOnUiThread(() -> {
                try {
                    ...
                    if (!"https".equalsIgnoreCase(uri.getScheme()) || uri.getHost() == null) { showConnector("Use the HTTPS URL printed by Cloudflare tunnel."); return; }
                    String clean = rawUrl.trim();
                    prefs.edit().putString(KEY_URL, clean).apply();
                    loadRemote(clean);
                } catch (Exception e) { showConnector("Invalid URL."); }
            });
        }
        @JavascriptInterface public void resetConnection() { runOnUiThread(() -> { prefs.edit().remove(KEY_URL).apply(); showConnector(""); }); }
        @JavascriptInterface public void printCurrent() { runOnUiThread(() -> { PrintManager pm = (PrintManager)getSystemService(Context.PRINT_SERVICE); pm.print("Chhotu_AI", webView.createPrintDocumentAdapter("Chhotu_AI"), null); }); }
    }
}
